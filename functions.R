...
print.hist.gauss<-function(x, path_output, coord, tempo) {
  ...
  hist(x, main= paste (tempo, "_Histogram_", coord , sep = ''), xlab = asse, probability = TRUE, col = color, breaks=nclass.FD(x)/2)
...
printBlock.hist.gauss<-function(x, path_output, coord, block, dim_bl,tempo) {
  ...
  hist(block_x, main= paste (tempo, "_Histogram_", coord , sep = ''), xlab = asse, probability = TRUE, col = color, breaks=nclass.FD(x)/2)
